       ...
       implements Serializable, Cloneable, Map <K, V>
{
    ...
    public Set <Map.Entry <K, V>> entrySet ()
    {
        ...
        for (int i = 0; i < hTable.size(); i++) {
           // System.out.println("number of keys " +hTable.get(i).nKeys );
                for (int j = 0; j < hTable.get(i).nKeys; j++) {
                    //System.out.println("key : " + hTable.get(i).key[j]);
                    //System.out.println("value : " + hTable.get(i).value[j]);
    ...
    private void split ()
    {
        ...
        List<V> tempValues = new ArrayList<>();

        Bucket tempBucket = splitBucket;
        while (tempBucket != null) {
        for(int i = 0; i < tempBucket.nKeys; i++) {
            tempKeys.add(tempBucket.key[i]);
            tempValues.add(tempBucket.value[i]);
        }
        tempBucket = tempBucket.next;
    }   

    splitBucket.next = null;
    splitBucket.nKeys = 0;
    

    if((isplit == hTable.size() / 2)) {
        isplit = 0; 
        mod1 *= 2; 
        mod2 = mod1 * 2;
    } else {
    ...
    for(K key_ : tempKeys) {
        ...
        var bucketHome = hTable.get(hash);
        tempBucket = bucketHome;
        do {
        ...
        if (tempBucket.nKeys < SLOTS) {
            tempBucket.add (tempKeys.get(i), tempValues.get(i));
        } else {
            var newBucket = new Bucket();
            newBucket.add(tempKeys.get(i), tempValues.get(i));
    ...
    {
        //out.println(Math.abs(key.hashCode() % mod1));
